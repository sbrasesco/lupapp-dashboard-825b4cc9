...
import { updateDeliveryCompany } from '../services/superAdminDeliveryService';
import { useToast } from "@/components/ui/use-toast";

const EditCompanyForm = ({ company, onClose, onSuccess }) => {
  ...
  // Cargar datos de la empresa cuando el componente se monta
  useEffect(() => {
    if (company) {
      setFormData({
        name: company.name || '',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await updateDeliveryCompany(company._id, formData);
      
      toast({
        title: "Éxito",
        description: "Empresa de delivery actualizada correctamente",
      });
      ...
      toast({
        title: "Error",
        description: error.message || "No se pudo actualizar la empresa de delivery",
        variant: "destructive",
      });
    } finally {
  ...
  return (
    <div className="glass-container p-6 rounded-lg animate-in fade-in duration-300">
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-xl font-semibold text-white">Editar Empresa de Delivery</h2>
        <Button 
          variant="ghost" 
          size="icon" 
          onClick={onClose}
