...
export const DispatchHours = ({ 
  ...
  const handleSave = async () => {
    try {
      const formattedData = formatScheduleData(shifts, 'dispatch');
      await updateSchedule(subDomain, localId, formattedData, accessToken);
      toast.success("Horario para reparto guardado correctamente");
    } catch (error) {
      toast.error("Error al guardar el horario para reparto");
    }
  };
  ...
  return (
    <Card className="bg-cartaai-black/30 p-4 rounded-lg backdrop-blur-sm">
      <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
        <CardTitle className="text-gray-700">Horario para Reparto de Pedidos</CardTitle>
        <Button 
          onClick={onSave}
